    ...
    def post_processing(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                ...
                if isinstance(post_process_cfg.NMS_CONFIG.NMS_THRESH, float):
                    ...
                    for cls in range(1, self.num_class + 1):
                        cls_mask = (label_preds == cls)
                        cls_box_scores = cls_preds[cls_mask]
                        cls_box_preds = box_preds[cls_mask]

                        if cls == 1:
                            nms_thresh = post_process_cfg.NMS_CONFIG.NMS_THRESH.CAR
                        elif cls == 2:
                            nms_thresh = post_process_cfg.NMS_CONFIG.NMS_THRESH.PED
                        elif cls == 3:
                            nms_thresh = post_process_cfg.NMS_CONFIG.NMS_THRESH.CYC
                        ...
                        selected_indices.extend(torch.nonzero(cls_mask, as_tuple=False).view(-1)[selected].tolist())
                    selected_indices = torch.tensor(selected_indices, device=cls_preds.device, dtype=torch.long)
                    selected_scores = cls_preds[selected_indices]
                    ...
                    final_boxes = box_preds[selected_indices]
