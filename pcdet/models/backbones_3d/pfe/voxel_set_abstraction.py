...
def sample_points_with_roi2(rois, points, r_add=2, h_add=1):
    """
    使用真值框的圆柱体过滤点云数据，采用向量化操作以提高效率。

    Args:
        point_cloud (torch.Tensor): 点云数据，形状为 (N, 5)，每个点包含 (x, y, z, rcs, v)。
        gt_boxes (torch.Tensor): 真值框数据，形状为 (M, 6)，每个真值框包含 (x, y, z, l, w, h)。
        r_add (float): 给真值框的半径增加的固定值。
        h_add (float): 给真值框的高度增加的固定值。

    Returns:
        filtered_point_cloud (torch.Tensor): 过滤后的点云数据。
        mask (torch.Tensor): 掩码，表示每个点是否被保留，形状为 (N,)。
    """

    # 真值框的中心点和大小
    gt_centers = rois[:, :3]  # 形状为 (M, 3)
    gt_dims = rois[:, 3:]  # 形状为 (M, 3)

    # 计算圆柱的半径和高度
    radii_cylinder = torch.max(gt_dims[:, 0], gt_dims[:, 1]) / 2 + r_add  # 半径为长宽最大值的一半 + 固定值
    heights_cylinder = gt_dims[:, 2] + h_add  # 高度为真值框高度 + 固定值

    # 扩展点云坐标用于与多个真值框并行计算
    points_xyz_exp = points.unsqueeze(1)  # 形状变为 (N, 1, 3)
    gt_centers_exp = gt_centers.unsqueeze(0)  # 形状变为 (1, M, 3)

    # 计算每个点与每个真值框中心的水平距离
    d_xy = torch.sqrt((points_xyz_exp[:, :, 0] - gt_centers_exp[:, :, 0]) ** 2 +
                      (points_xyz_exp[:, :, 1] - gt_centers_exp[:, :, 1]) ** 2)  # 形状 (N, M)

    # 获取 z 轴的上下限
    z_min = gt_centers[:, 2] - heights_cylinder / 2  # 形状为 (M,)
    z_max = gt_centers[:, 2] + heights_cylinder / 2  # 形状为 (M,)

    # 扩展 z 轴的范围以匹配点云
    z_min_exp = z_min.unsqueeze(0)  # 形状变为 (1, M)
    z_max_exp = z_max.unsqueeze(0)  # 形状变为 (1, M)

    # 计算 z 轴高度范围内的掩码
    mask_z = (points_xyz_exp[:, :, 2] >= z_min_exp) & (points_xyz_exp[:, :, 2] <= z_max_exp)  # 形状 (N, M)

    # 计算水平距离的掩码
    mask_xy = d_xy <= radii_cylinder.unsqueeze(0)  # 形状为 (N, M)

    # 综合水平和垂直方向的掩码
    mask = mask_xy & mask_z  # 形状为 (N, M)

    # 对每个点，任意真值框满足条件即保留该点
    point_mask = mask.any(dim=1)  # 形状为 (N,)
...
class VoxelSetAbstraction(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
        ...
        for src_name in self.model_cfg.FEATURES_SOURCE:
            if src_name in ['bev', 'raw_points', 'cluter_points']:
                continue
        ...
        if 'cluter_points' in self.model_cfg.FEATURES_SOURCE:
            self.SA_rawpoints, cur_num_c_out = pointnet2_stack_modules.build_local_aggregation_module(
                input_channels=num_rawpoint_features - 3, config=SA_cfg['cluter_points']
            )
    ...
    def sectorized_proposal_centric_sampling3(self, roi_boxes, points):
        """
        Args:
            roi_boxes: (M, 7 + C)
            points: (N, 3)
        ...
        sampled_points, _ = sample_points_with_roi2(
            rois=roi_boxes, points=points,
            r_add=self.model_cfg.CDAM_SAMPLING.SAMPLE_RADIUS_R,
            h_add=self.model_cfg.CDAM_SAMPLING.SAMPLE_RADIUS_H
        )

        if len(sampled_points) > self.model_cfg.NUM_KEYPOINTS:
            sampled_points = sector_fps(
                points=sampled_points, num_sampled_points=self.model_cfg.NUM_KEYPOINTS,
                num_sectors=self.model_cfg.CDAM_SAMPLING.NUM_SECTORS
            )
        return sampled_points

    def sectorized_proposal_centric_sampling2(self, roi_boxes, points):
        """
        Args:
            roi_boxes: (M, 7 + C)
            points: (N, 3)
        ...
        sampled_points, _ = sample_points_with_roi2(
            rois=roi_boxes, points=points,
            r_add=self.model_cfg.MCS_SAMPLING.SAMPLE_RADIUS_R,
            h_add=self.model_cfg.MCS_SAMPLING.SAMPLE_RADIUS_H
        )
        sampled_points = sector_fps(
            points=sampled_points, num_sampled_points=self.model_cfg.NUM_KEYPOINTS,
            num_sectors=self.model_cfg.MCS_SAMPLING.NUM_SECTORS
        )
        return sampled_points
    ...
    def get_sampled_points(self, batch_dict):
        ...
        elif self.model_cfg.POINT_SOURCE == 'cluter_points':
            src_points = batch_dict['cluter_points'][:, 1:4]
            batch_indices = batch_dict['cluter_points'][:, 0].long()
            src_points2 = batch_dict['points'][:, 1:4]
            batch_indices2 = batch_dict['points'][:, 0].long()
        ...
        for bs_idx in range(batch_size):
            ...
            elif self.model_cfg.SAMPLE_METHOD == 'MCS':
                cur_keypoints = self.sectorized_proposal_centric_sampling2(
                    roi_boxes=batch_dict['rois'][bs_idx], points=sampled_points[0]
                )
                bs_idxs = cur_keypoints.new_ones(cur_keypoints.shape[0]) * bs_idx
                keypoints = torch.cat((bs_idxs[:, None], cur_keypoints), dim=1)

            elif self.model_cfg.SAMPLE_METHOD == 'CDAM':
                bs_mask2 = (batch_indices2 == bs_idx)  ##CDAM!!!
                sampled_points2 = src_points2[bs_mask2].unsqueeze(dim=0)  ##CDAM!!!
                cur_keypoints1 = self.sectorized_proposal_centric_sampling3(
                    roi_boxes=batch_dict['rois'][bs_idx], points=sampled_points[0]
                )

                cur_pt_idxs = pointnet2_stack_utils.farthest_point_sample(
                    sampled_points2[:, :, 0:3].contiguous(), self.model_cfg.NUM_KEYPOINTS-len(cur_keypoints1)
                ).long()

                if sampled_points2.shape[1] < self.model_cfg.NUM_KEYPOINTS-len(cur_keypoints1):
                    times = int((self.model_cfg.NUM_KEYPOINTS-len(cur_keypoints1)) / sampled_points2.shape[1]) + 1
                    non_empty = cur_pt_idxs[0, :sampled_points2.shape[1]]
                    cur_pt_idxs[0] = non_empty.repeat(times)[:self.model_cfg.NUM_KEYPOINTS-len(cur_keypoints1)]

                cur_keypoints2 = sampled_points2[0][cur_pt_idxs[0]]
                cur_keypoints = torch.cat((cur_keypoints1, cur_keypoints2), dim=0)
    ...
    def forward(self, batch_dict):
        ...
        point_features_list = []
        if 'bev' in self.model_cfg.FEATURES_SOURCE:
            # if spatial_features_stride in batch_dict:
            #     features_stride = batch_dict['spatial_features_stride']
            # else:
            #     features_stride = 1
        ...
        if 'cluter_points' in self.model_cfg.FEATURES_SOURCE:
            ...
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                ...
                xyz_bs_idxs=raw_points[:, 0],
                new_xyz=new_xyz, new_xyz_batch_cnt=new_xyz_batch_cnt,
                filter_neighbors_with_roi=self.model_cfg.SA_LAYER['cluter_points'].get('FILTER_NEIGHBOR_WITH_ROI', False),
                radius_of_neighbor=self.model_cfg.SA_LAYER['cluter_points'].get('RADIUS_OF_NEIGHBOR_WITH_ROI', None),
                rois=batch_dict.get('rois', None)
            )
            point_features_list.append(pooled_features)
...
class VoxelSetAbstraction_cda(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
                 num_rawpoint_features=None, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        ...
        c_in = 0
        for src_name in self.model_cfg.FEATURES_SOURCE:
            if src_name in ['bev', 'raw_points', 'cluster_points']:
                continue
            self.downsample_times_map[src_name] = SA_cfg[src_name].DOWNSAMPLE_FACTOR
        ...
        if 'cluster_points' in self.model_cfg.FEATURES_SOURCE:
            self.SA_clusterpoints, cur_num_c_out = pointnet2_stack_modules.build_local_aggregation_module(
                input_channels=num_rawpoint_features - 3, config=SA_cfg['cluster_points']
            )
    ...
    def get_sampled_points(self, batch_dict):
        ...
        if self.model_cfg.POINT_SOURCE == 'raw_points':
            src_points = batch_dict['points'][:, 1:4]
            batch_indices = batch_dict['points'][:, 0].long()
        elif self.model_cfg.POINT_SOURCE == 'cluster_points':
            src_points = batch_dict['cluster_points'][:, 1:4]
            batch_indices = batch_dict['cluster_points'][:, 0].long()
            src_points2 = batch_dict['points'][:, 1:4]
            batch_indices2 = batch_dict['points'][:, 0].long()
        elif self.model_cfg.POINT_SOURCE == 'voxel_centers':
            src_points = common_utils.get_voxel_centers(
        ...
        for bs_idx in range(batch_size):
            ...
            elif self.model_cfg.SAMPLE_METHOD == 'CDA':
                bs_mask2 = (batch_indices2 == bs_idx)  ##CDA!!!
                sampled_points2 = src_points2[bs_mask2].unsqueeze(dim=0)  ##CDA!!!
                cur_keypoints1 = self.sectorized_proposal_centric_sampling3(
                    roi_boxes=batch_dict['rois'][bs_idx], points=sampled_points[0]
                )
    ...
    def forward(self, batch_dict):
        ...
        if 'cluster_points' in self.model_cfg.FEATURES_SOURCE:
            ...
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                batch_size=batch_size, aggregate_func=self.SA_clusterpoints,
                xyz=raw_points[:, 1:4],
                xyz_features=raw_points[:, 4:].contiguous() if raw_points.shape[1] > 4 else None,
                xyz_bs_idxs=raw_points[:, 0],
                new_xyz=new_xyz, new_xyz_batch_cnt=new_xyz_batch_cnt,
                filter_neighbors_with_roi=self.model_cfg.SA_LAYER['cluster_points'].get('FILTER_NEIGHBOR_WITH_ROI', False),
                radius_of_neighbor=self.model_cfg.SA_LAYER['cluster_points'].get('RADIUS_OF_NEIGHBOR_WITH_ROI', None),
                rois=batch_dict.get('rois', None)
            )
            point_features_list.append(pooled_features)
