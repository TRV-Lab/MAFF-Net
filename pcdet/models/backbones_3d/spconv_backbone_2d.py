...
class PillarBackBone8x_DU(nn.Module):
...
class PillarBackBone8x_dual(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
    ...
    def forward(self, batch_dict):
        lidar_pillar_features, lidar_pillar_coords = batch_dict['lidar_pillar_features'], batch_dict[
            'lidar_pillar_coords']
        radar_pillar_features, radar_pillar_coords = batch_dict['radar_pillar_features'], batch_dict[
            'radar_pillar_coords']
        batch_size = batch_dict['batch_size']
        lidar_input_sp_tensor = spconv.SparseConvTensor(
            features=lidar_pillar_features,
            indices=lidar_pillar_coords.int(),
            spatial_shape=self.sparse_shape,
            batch_size=batch_size
        )

        radar_input_sp_tensor = spconv.SparseConvTensor(
            features=radar_pillar_features,
            indices=radar_pillar_coords.int(),
            spatial_shape=self.sparse_shape,
            batch_size=batch_size
        )

        radar_x_conv1 = self.conv1(radar_input_sp_tensor)
        radar_x_conv2 = self.conv2(radar_x_conv1)
        radar_x_conv3 = self.conv3(radar_x_conv2)
        radar_x_conv4 = self.conv4(radar_x_conv3)
        radar_x_conv4 = radar_x_conv4.dense()
        radar_x_conv5 = self.conv5(radar_x_conv4)

        batch_dict.update({
            'radar_multi_scale_2d_features': {
                'x_conv1': radar_x_conv1,
                'x_conv2': radar_x_conv2,
                'x_conv3': radar_x_conv3,
                'x_conv4': radar_x_conv4,
                'x_conv5': radar_x_conv5,
            }
        })
        lidar_x_conv1 = self.conv1(lidar_input_sp_tensor)
        lidar_x_conv2 = self.conv2(lidar_x_conv1)
        lidar_x_conv3 = self.conv3(lidar_x_conv2)
        lidar_x_conv4 = self.conv4(lidar_x_conv3)
        lidar_x_conv4 = lidar_x_conv4.dense()
        lidar_x_conv5 = self.conv5(lidar_x_conv4)

        batch_dict.update({
            'lidar_multi_scale_2d_features': {
                'x_conv1': lidar_x_conv1,
                'x_conv2': lidar_x_conv2,
                'x_conv3': lidar_x_conv3,
                'x_conv4': lidar_x_conv4,
                'x_conv5': lidar_x_conv5,
            }
        })
        batch_dict.update({
            'multi_scale_2d_strides': {
