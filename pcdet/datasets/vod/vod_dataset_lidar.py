...
class VodDatasetLidar(DatasetTemplate):
    ...
    def __getitem__(self, index):
        ...
        if "calib_matricies" in get_item_list:
            ...
            input_dict["lidar2camera"], input_dict["camera_intrinsics"], input_dict["camera2lidar"], input_dict["lidar2image"] = kitti_utils.calib_to_matricies(calib)
...
def create_vod_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = VodDatasetLidar(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_vod_infos':
        ...
        #先使用pathlib库的Path类读取到当前执行的python文件的路径，其中魔术变量__file__代表当前的文件。
        #然后通过.parent()找到文件所在的目录 “/": 有助于拼接子目录，类似,os.path.join()
        #再用resolve()解析成文件字符串
        ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
        # ROOT_DIR ='/home/zhanghaoming/projects/VFF'
        create_vod_infos(
            dataset_cfg=dataset_cfg,
            class_names=['Car', 'Pedestrian', 'Cyclist'],
            data_path=ROOT_DIR / 'data' / 'vod_all'/'lidar',
            save_path=ROOT_DIR / 'data' / 'vod_all'/'lidar'
        )

# python -m pcdet.datasets.vod.vod_dataset_lidar create_vod_infos tools/cfgs/dataset_configs/vod_dataset_lidar.yaml