...
class DatasetTemplate(torch_data.Dataset):
    ...
    def set_lidar_aug_matrix(self, data_dict):
        ...
        if 'points' in data_dict:
            if isinstance(data_dict['points'], list):
                data_dict['lidar_aug_matrix'] = [lidar_aug_matrix, lidar_aug_matrix]
            else:
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        ret = {}
        batch_size_ratio = 1
        if 'lidar_voxels' in data_dict:
            for key, val in data_dict.items():
                try:
                    if key in ['lidar_voxels', 'lidar_voxel_num_points']:
                        ret[key] = np.concatenate(val, axis=0)
                    elif key in ['radar_voxels', 'radar_voxel_num_points']:
                        ret[key] = np.concatenate(val, axis=0)
                    elif key in ['lidar_points', 'lidar_voxel_coords']:
                        lidar_coors = []
                        for lidar_i, lidar_coor in enumerate(val):
                            lidar_coor_pad = np.pad(lidar_coor, ((0, 0), (1, 0)), mode='constant', constant_values=lidar_i)
                            lidar_coors.append(lidar_coor_pad)
                        ret[key] = np.concatenate(lidar_coors, axis=0)
                    elif key in ['radar_points', 'radar_voxel_coords']:
                        radar_coors = []
                        for radar_i, radar_coor in enumerate(val):
                            radar_coor_pad = np.pad(radar_coor, ((0, 0), (1, 0)), mode='constant', constant_values=radar_i)
            ...
            for key, val in data_dict.items():
                try:
                    if key in ['voxels', 'voxel_num_points']:
                        ...
                        ret[key] = np.concatenate(val, axis=0)
                    elif key in ['dbscan1_points', 'dbscan2_points', 'pseudo_points', 'points', 'voxel_coords']:
                        coors = []
                        if isinstance(val[0], list):
                            val =  [i for item in val for i in item]
