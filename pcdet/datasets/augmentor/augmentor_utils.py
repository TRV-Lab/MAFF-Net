...
def fusion_random_flip_along_x(gt_boxes, lidar_points, radar_points):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        lidar_points: (M, 3 + C)
    ...
    return gt_boxes, lidar_points, radar_points, enable
...
def fusion_random_flip_along_y(gt_boxes, lidar_points, radar_points):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        lidar_points: (M, 3 + C)
...
def global_rotation(gt_boxes, points, rot_range, return_rot=False):
    ...
    if isinstance(points, list):
        points[0] = common_utils.rotate_points_along_z(points[0][np.newaxis, :, :], np.array([noise_rotation]))[0]
        points[1] = common_utils.rotate_points_along_z(points[1][np.newaxis, :, :], np.array([noise_rotation]))[0]
    else:
...
def fusion_global_rotation(gt_boxes, lidar_points, radar_points, rot_range):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    """
    noise_rotation = np.random.uniform(rot_range[0], rot_range[1])
    lidar_points = common_utils.rotate_points_along_z(lidar_points[np.newaxis, :, :], np.array([noise_rotation]))[0]
    ...
    return gt_boxes, lidar_points, radar_points, noise_rotation
...
def global_scaling(gt_boxes, points, scale_range, return_scale=False):
    ...
    if isinstance(points, list):
        points[0][:, :3] *= noise_scale
        points[1][:, :3] *= noise_scale
    else:
...
def fusion_global_scaling(gt_boxes, lidar_points, radar_points, scale_range):
    """
    Args:
        gt_boxes: (N, 7), [x, y, z, dx, dy, dz, heading]
        points: (M, 3 + C),
    ...
    """
    if scale_range[1] - scale_range[0] < 1e-3:
        return gt_boxes, lidar_points, radar_points
    noise_scale = np.random.uniform(scale_range[0], scale_range[1])
    lidar_points[:, :3] *= noise_scale
    radar_points[:, :3] *= noise_scale
    gt_boxes[:, :6] *= noise_scale
    return gt_boxes, lidar_points, radar_points, noise_scale
