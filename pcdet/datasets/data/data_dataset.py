...
from ...utils import box_utils, calibration_data, common_utils, object3d_data
from ..dataset import DatasetTemplate


class DataDataset(DatasetTemplate):
    ...
    def get_label(self, idx):
        ...
        return object3d_data.get_objects_from_label(label_file)

    def get_calib(self, idx):
        calib_file = self.root_split_path / 'calib' / ('%s.txt' % idx)
        assert calib_file.exists()
        return calibration_data.Calibration(calib_file)
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            calib_info = {'P2': P2,  'Tr_velo_to_cam': V2C_4x4}
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        db_info_save_path = Path(self.root_path) / ('data_dbinfos_%s.pkl' % split)
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = DataDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
    train_split, val_split = 'train', 'val'

    train_filename = save_path / ('data_infos_%s.pkl' % train_split)
    val_filename = save_path / ('data_infos_%s.pkl' % val_split)
    trainval_filename = save_path / 'data_infos_trainval.pkl'
    test_filename = save_path / 'data_infos_test.pkl'
...
if __name__ == '__main__':
    import sys
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_data_infos':
        import yaml
