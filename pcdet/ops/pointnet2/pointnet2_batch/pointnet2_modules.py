...
class Vote_layer_new(nn.Module):
...
class PointnetSAModuleFSMSGWithImage(PointnetSAModuleFSMSG):
    ...
    def __init__(self, *,
        ...
        self.fuse3d_mlp = nn.Sequential(
            nn.Conv1d(in_channels_2d + in_channels_3d, in_channels_3d, kernel_size=1, bias=False),
    ...
    def forward(self,
        ...
        if image_features is not None:
            ...
            for bs_idx in range(batch_size):
                ...
                cur_point_image_features = torch.zeros((len(valid_mask), cur_image_features.shape[-1]),
                                                       dtype=cur_image_features.dtype, device=cur_image_features.device)
                valid_cur_point_image_features = common_utils.bilinear_interpolate_torch(cur_image_features,
                                                                                         scan_C2[valid_mask == 1, 0],
                                                                                         scan_C2[valid_mask == 1, 1])
            ...
            for bs_idx in range(batch_size):
                ...
                valid_point_mask = (bs_uv[:, 0] >= 0) & (bs_uv[:, 0] < image_features[0].shape[2]) & (
                        bs_uv[:, 1] >= 0) & (bs_uv[:, 1] < image_features[0].shape[1])
                ...
                gather_3d_features = torch.sparse_coo_tensor(bs_uv_grid, bs_new_features, (
                    image_features_w, image_features_h, new_features.shape[1]))
