...
def parse_config():
    ...
    parser.add_argument('--extra_tag', type=str, default='default',
                        help='extra tag for this experiment')  # 给此次训练加s一个额外标签,例如'kradar'，这个会在输出的工作路径下创建一个kradar文件夹，所有生成的文件均在里面
    ...
    parser.add_argument('--max_ckpt_save_num', type=int, default=21,
                        help='max number of saved checkpoint')  # <= num_epochs_to_eval
...
def main():
    ...
    if dist_train:
        model = nn.parallel.DistributedDataParallel(model, device_ids=[
            cfg.LOCAL_RANK % torch.cuda.device_count()])  # find_unused_parameters=True
